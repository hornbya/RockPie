...
//Component analysis for SEM images
...
//(Rock Pie calcs) and a macro using the Read and Write Excel plugin for easy data crunching.

//The macro is expanded from the surface salts segmentation macro described here
//Casas et al. Bull Volcanol 84, 3 (2022) - https://doi.org/10.1007/s00445-021-01519-3
//and available here https://github.com/hornbya/Ash-surface-salts
//***********************************************************************
...
//reduce noise and set the  threshold for surface particles
	function manualthreshold() { 
		Dialog.create("Non-local means denoising");
		Dialog.addCheckbox("Run non-local means denoising?", 0);
		...
		if (nlmd==1) {
			...
			Dialog.create("Accept NLMD?");
			Dialog.addCheckbox("Accept Non-local Means Denoising?", 0);
			Dialog.show();
...
//manual editing of threshold area
	function fillparticles() {
		...
		waitForUser("Use Paintbrush to define "+particles+" only (if necessary)..., then press OK");
...
//**************************************
	...
	Dialog.addMessage("Enter description of components to segment");
